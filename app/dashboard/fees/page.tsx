...
import { WalletCards, CheckCircle2, Clock3, AlertCircle } from "lucide-react";
...
export default function FeesPage() {
  ...
  const canManageFees = currentRole === "admin";
  ...
  const handleDeleteFee = async (id: string) => {
    ...
    setFeeList((prev) => prev.filter((fee) => fee.id !== id));
  };

  const classOptions = useMemo(() => {
    const classes = Array.from(
      new Set(feeList.map((item) => item.className).filter(Boolean))
    );
  ...
  }, [feeList, searchTerm, classFilter, statusFilter]);

  const totalRecords = feeList.length;

  const totalDue = feeList.reduce((sum, item) => {
    return sum + Number(item.amountDue || 0);
  }, 0);

  const totalPaid = feeList.reduce((sum, item) => {
    return sum + Number(item.amountPaid || 0);
  }, 0);

  const totalBalance = feeList.reduce((sum, item) => {
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">
              Search, filter, and manage all student fee payment records.
            </p>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:flex">
            <input
              type="text"
              placeholder="Search by student, ID, or term..."
              value={searchTerm}
          ...
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="text-sm text-slate-600">
                Showing {filteredFees.length} payment
